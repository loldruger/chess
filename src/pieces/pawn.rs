        ...
        match self.color {
            Color::Black => {
                ...
                if board.is_empty(Square::from_position((current_file, target_rank))) {
                    valid_moves.push((Square::from_position((current_file, target_rank)), CaptureStatus::NotCapturable));
                }
                // Double move from the starting rank
                if current_rank == 6 && board.is_empty(Square::from_position((current_file, target_rank))) {
                    valid_moves.push((Square::from_position((current_file, target_rank - 1)), CaptureStatus::NotCapturable));
                }
